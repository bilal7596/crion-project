...
let primaryObject = {};
export const EditAssetSpecifications = ({ getData, data,getChangedValues,changedData }) => {
  const user = getUser();
  ...
  const handleChange = ({ e, label }) => {
    ...
    setFormData((prevData) => {
      ...
      if (
        ...
        console.log(formData?.warrantyPeriodDurr,value,'warrantyPeriodNumber')
        temp = {
          ...prevData,
          [name]: value,
  ...
  const handleGetUploadedfiles = (val) => {
    if(val?.name === "needToDeleteDocuments"){
      ...
      let temp = {
        ...formData,
        assetTermsAndConditionsToDelete: filesTobeDeleted,
      };
      let changeIn = {
        ...changedData,
        assetTermsAndConditionsToDelete: filesTobeDeleted,
  ...
  console.log(changedData,"chhcch");
  useEffect(() => {
    if (
            ...
            <div className={Styles.warrantyDetailsRightContainer}>
              <div>
                <FileUploader
                  ...
                  fileSizeInMB={10}
                  defaultValue={formData?.assetTermsAndConditions}
                  localPreviousPageFiles={changedData?.assetTermsAndConditions}
