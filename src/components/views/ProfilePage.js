...
import { getUserSessions, postUserFilters } from "../../Api/User/UserApi";
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    getUserSessions(userLoginDetail?.userId)
      .then((res) => {
        console.log("getUserSessions RESPONSE", res);
        setuserSessions(res?.data?.rows);
      })
      .catch((err) => console.log("getUserSessions ERROR", err));
      postUserFilters({userId: userLoginDetail?.userId}).then((res) => {
        console.log("postUserFilters RESPONSE", res);
        // setuserSessions(res?.data?.rows);
        setuserDetail(res?.data?.users[0])
      })
      .catch((err) => console.log("postUserFilters ERROR", err));
  }, []);
