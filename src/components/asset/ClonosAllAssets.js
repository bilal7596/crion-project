...
export default function ClonosAllAssets() {
  ...
  const [allAssetLayoutImageSource, setAllAssetLayoutImageSource] = useState([]);
  ...
  const columns = [
...

    {
      ...
      renderCell: (value) => {
        ...
        let dimentions = imageasset.height + " x " + imageasset.width + " px";
        ...
        return (
          <div>
            ...
            >
              <Avatar
                onClick={(bool, event) => {   
                  console.log("dimentions::",imageasset.height);
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <>
            {user.permissions.includes("ast004") && (
            ...
            {user.permissions.includes("doc007") ? (
          ...
          <>
            <Tooltip title="Engineering Data">
              <IconButton
                ...
                style={{
                  display: user.permissions.includes("egd005")
                    ? "block"
                    : "none",
                }}
              >
  ...
  useEffect(() => {
    getAssetFilters()
      .then((res) => {
        ...
        const orderData = {
          ...
          assetCategory: res.data.assetCategory,
    ...
    getAllAssetImageDetails()
      ...
      getAllAssetLayoutImageDetails()
      .then((res) => {
        console.log("getAllAssetLayoutImageDetails res", res);
        setAllAssetLayoutImageSource(res.data.result);
      })
      .catch((err) => {
        if(err.response.data.status=== 401 && JSON.parse(localStorage.getItem("loginUser")) !== null){
          dispatch(commonActions.handleExpiryAlert(true));
  ...
  const handleUploadSheet = (e) => {
    ...
    function compareKeys(a, b, notMandatory) {
      var aKeys = Object.keys(a).filter((field)=>{
        if(!notMandatory.includes(field)){
          return field;
        }
        
      }).sort();
      var bKeys = Object.keys(b).filter((field)=>{
        if(!notMandatory.includes(field)){
          return field;
        }
    ...
    if (e.target.files) {
      ...
      reader.onload = (e) => {
        ...
        if (convertedJson.length > 0) {
          errorData = compareKeys(convertedJson[0], fileTemp, ["assetId", "shortDescription"]);
        }
        if (errorData) {
              ...
              >
                ...
                <label htmlFor="contained-button-file">
                  <Button
                    ...
                    component="span"
                    style={{
                      display: user.permissions.includes("ast001") ? "block" : "none",
                    }}
                  >
                    Upload
                  </Button>
        ...
        <div style={{ height: "700px", width: "100%" }}>
          {selectedRows.length > 0 && (
            ...
            >
              ...
              <div>
                {selectedRows.length < 2 && user.permissions.includes("ast002") && (
                  <>
                    <Tooltip title="Edit">
                ...
                {user.permissions.includes("ast003") ? (
          ...
          <DataGrid
            ...
            checkboxSelection={
              user.permissions.includes("ast002") || user.permissions.includes("ast003")
                ? true
                : false
            }
            disableColumnMenu
